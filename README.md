...
- Install PySpin first. This is tricky because there's a PyPi package online called _pyspin_ which is not the right package. PySpin actually comes downloaded with Spinnaker SDK IIRC and you have to install the correct one for your computer hardware with pip from wheels.
- However, since I can't find it on the upstairs lab computer/old version of Spinnaker, it turns out you can install it with `pip install spinnaker-python`. (This may or may not work with these scripts. My experience is that it doesn't.)
- Also requires pyserial (installed w Conda or pip), opencv, seaborn, etc
...
The serotonin dynamics may be pretty slow. So it could be interesting to look at these effects post-hoc (e.g. how likely is it for another attack to start while the light is off).
